...
pinned git revision into the install prefix, run `meson setup` to fetch the pinned frida wraps, and
generate the local dev keypair. It SHALL work identically on a developer machine and in CI.

#### Scenario: Fresh clone is provisioned
- **WHEN** a developer runs `make init` on a clean clone
- **THEN** the OS-package tools and `uncrustify` are installed, `io.elementary.vala-lint` is built
  from its pinned revision and installed, the frida wraps are fetched, and a dev keypair exists
