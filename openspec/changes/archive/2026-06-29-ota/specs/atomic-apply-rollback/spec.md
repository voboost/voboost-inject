...
NOTE: agents are individual files referenced by the daemon manifest; the atomic
unit of an agent update is the manifest swap. The core binary cannot be replaced
while running, so a core update takes effect at the next daemon (re)start (the
daemon self-shuts down and init restarts the new binary), not in place.
